...
export default function BizMenusPage() {
    ...
    const loadMenus = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const { data } = await bizGet<ApiResponse<PageResponse<SystemMenuDTO>>>(
                bizId,
                '/menus',
                '/api/menus',
                { params: { page, size: pageSize, sort: 'id,asc' } }
            );
            if (data?.payload) {
                const p = data.payload;
                const content = (p && 'content' in p) ? p.content : (Array.isArray(p) ? p : []);
                const total = (p && 'totalElements' in p) ? p.totalElements : content.length;
                const totalP = (p && 'totalPages' in p) ? p.totalPages : Math.ceil(total / pageSize) || 1;
                setMenus(content);
    ...
    const filteredMenus = menus.filter(
        m => m.menuName?.toLowerCase().includes(searchQuery.toLowerCase()) || m.description?.toLowerCase().includes(searchQuery.toLowerCase()),
    );
    ...
    const handleSave = async () => {
        if (!formData.menuName.trim()) { setFormError('Menu name is required'); return; }
        if (!bizId) return;
        setIsSaving(true);
        setFormError('');
        try {
            if (editingMenu) {
                await bizPut(bizId, `/menus/${editingMenu.id}`, `/api/menus/${editingMenu.id}`, formData);
            } else {
                await bizPost(bizId, '/menus', '/api/menus', formData);
            }
            setShowModal(false);
            loadMenus();
        } catch (error: any) {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await bizDelete(bizId, `/menus/${deleteTarget.id}`, `/api/menus/${deleteTarget.id}`);
    ...
    if (!bizId) {
        return (
            <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-muted)' }}>
                <MenuIcon size={56} style={{ opacity: 0.2, marginBottom: 16 }} />
    ...
    return (
        <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
                    <div>
                        ...
                        <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: 0 }}>
                            Create and manage menu items for your business; assign them to roles and staff ({totalElements} total)
                        </p>
                    </div>
            ...
            <div className="animate-fade-in-up delay-2">
                {isLoading ? (
                    ...
                    <div className="glass-card" style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
                        ...
                        <p style={{ fontSize: 14 }}>Add menu items to use as permissions for roles and staff</p>
                    </div>
                ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
                        {filteredMenus.map((menu) => (
                            <div
                                ...
                                style={{ padding: '20px', border: '1px solid rgba(255,255,255,0.05)', transition: 'all 0.3s ease', position: 'relative' }}
                            >
                                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 4 }}>
                                        <button onClick={() => openEditModal(menu)} className="icon-btn" title="Edit"><Edit3 size={15} /></button>
                                        <button onClick={() => setDeleteTarget(menu)} className="icon-btn" title="Delete" style={{ color: '#f87171' }}><Trash2 size={15} /></button>
                                    </div>
                                </div>
                                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>{menu.menuName}</h3>
                                <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 40 }}>
                                    {menu.description || <span style={{ opacity: 0.4 }}>No description</span>}
                                </p>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <span className={`badge ${menu.status === 'ACTIVE' ? 'badge-success' : 'badge-warning'}`} style={{ fontSize: 11 }}>{menu.status}</span>
                                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>ID: #{menu.id}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 24, fontSize: 13, color: 'var(--text-muted)' }}>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowModal(false)}>
                    <div className="glass-card animate-fade-in-up" style={{ width: '100%', maxWidth: 520, padding: 0 }} onClick={(e) => e.stopPropagation()}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                            <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{editingMenu ? 'Edit Menu Item' : 'Create Menu Item'}</h2>
                        ...
                        <div style={{ padding: '24px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                <div>
                                    ...
                                    <input className="input-field" placeholder="e.g. Inventory, Sales" value={formData.menuName} onChange={(e) => setFormData({ ...formData, menuName: e.target.value })} style={{ width: '100%' }} />
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                    <div>
                                        <label className="input-label">Sort Order</label>
                                        <input type="number" className="input-field" value={formData.sorting} onChange={(e) => setFormData({ ...formData, sorting: parseInt(e.target.value) || 0 })} style={{ width: '100%' }} />
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <select className="input-field" value={formData.parentId || ''} onChange={(e) => setFormData({ ...formData, parentId: e.target.value })} style={{ width: '100%' }}>
                                ...
                                <div>
                                    ...
                                    <textarea className="input-field" placeholder="Description of this menu..." value={formData.description || ''} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={2} style={{ width: '100%', resize: 'none' }} />
                                </div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                            ...
                            <button className="btn-primary" onClick={handleSave} disabled={isSaving} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{isSaving && <Loader2 size={16} className="animate-spin-slow" />}{editingMenu ? 'Update Menu' : 'Create Menu'}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {deleteTarget && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setDeleteTarget(null)}>
                    <div className="glass-card animate-fade-in-up" style={{ width: '100%', maxWidth: 400, padding: 28, textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Delete Menu Item</h3>
                        <p style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 24 }}>Delete <strong style={{ color: 'var(--text-primary)' }}>{deleteTarget.menuName}</strong>?</p>
